...
resource "vault_pki_secret_backend_config_cluster" "rsa_config_cluster" {
  backend  = vault_mount.rsa.path
  path     = "https://vault.li.lab:8200/v1/rsa"
  aia_path = "https://vault.li.lab:8200/v1/rsa"
}

resource "vault_pki_secret_backend_config_acme" "rsa" {
  backend                  = vault_mount.rsa.path
  enabled                  = true
  allowed_issuers          = ["*"]
  allowed_roles            = ["*"]
